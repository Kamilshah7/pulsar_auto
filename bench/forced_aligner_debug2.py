...
class ForcedAligner:
    ...
    def snap_boundaries_hybrid(self, words, pipes_or_audio_path, audio_path=None, vad_spans=None, preloaded_audio=None):
        ...
        for i in range(len(refined) - 1):
            ...
            if w1["text"]=="but" and w2["text"]=="i":
                print("DBG_TOP", "p_start=",p_start,"p_end=",p_end,"w1end=",w1["end"],"w2start=",w2["start"])

            pause_dur = w2["start"] - p_end
            pause_check_start = p_end
            pipe_suggests_continuity = pause_dur < 0.100
            ...
            if is_true_pause:
                if w1["text"]=="but" and w2["text"]=="i":
                    print("DBG_BRANCH: true_pause=True")
                # ── TRUE PAUSE ───────────────────────────────────────────────
                ...
                if found_drop:
                    ...
                    if ends_in_stop(w1["text"]) and (w2["start"] - w1["end"]) < 0.400:
                        # Forward check across stop closure: if within the closure there is a SUSTAINED
                        # plosive burst spike >= speech_thresh (require a short hold so a single noisy
                        # sample isn't mistaken for the real release burst; window extended to 260ms to
                        # cover longer closures where the true burst lands beyond the old 130ms cap)
                        burst_lim = s2i(min(w2["start"] - 0.020, first_drop_i / sr + 0.260))
                        burst_hold = max(1, int(0.006 * sr))
                        burst_k = None
                        for k in range(first_drop_i, burst_lim):
                            if rms[k] >= speech_thresh:
                                window = rms[k: min(k + burst_hold, burst_lim)]
                                if len(window) >= burst_hold and np.mean(window) >= speech_thresh * 0.7:
                                    burst_k = k
                ...
                if pipe_suggests_continuity and ends_in_stop(w1["text"]):
                    # The CTC separator (pipe) emission already placed its end close to w2's raw
                    # start -- i.e. the model itself resolved the stop closure as belonging to w1,
                    # right up to the release. A from-scratch RMS silence scan anchored at w1's own
                    # (characteristically-early, for stop-final words) raw end can walk back into a
                    # mid-word closure dip and under-shoot that. Don't let it undershoot the pipe's
                    # own end estimate.
                    w1_end_t = max(w1_end_t, min(p_end, w2["start"] - 0.005))
                ...
                if w1["text"]=="but" and w2["text"]=="i":
                    print("DBG_BRANCH: true_pause=False (continuous branch)", "pause_dur=", pause_dur)
                # ── CONTINUOUS SPEECH ─────────────────────────────────────────
                ...
                # 10.5. Stop-final w1 into a non-vowel-initial w2 (e.g. "perfect -> yeah"): the CTC
                # blank/pipe emission reliably spans a stop closure up to the release, so trust p_end
                # rather than hunting for the deepest point of the closure -- the generic valley search
                # (next branch) finds the quietest point of the closure itself, not its end, and would
                # place the boundary too early, stranding the release burst on the wrong side.
                elif ends_in_stop(w1["text"]) and (p_end - p_start) >= 0.015:
                    bnd = p_end
                ...
                w2["start"] = round(bnd + 0.001, 4)
                if w1["text"]=="but" and w2["text"]=="i":
                    print("DBG_CONT_FINAL bnd=",bnd,"w1end=",w1["end"],"w2start=",w2["start"])
