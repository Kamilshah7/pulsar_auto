var bdiagnostic = Object.create(null);
if (!bdiagnostic.log) {
    bdiagnostic.log = function (...inputs) {
    };
    bdiagnostic.devLog = function (...inputs) {
    };
	bdiagnostic.exception = function (ex) {
        if (typeof Sentry !== 'undefined') {
			Sentry.captureException(ex);
		}
	};
}

var SendMessageToBackground = (function() {
    if (BROWSER_VALUES.SOURCE === 'chrome' || BROWSER_VALUES.SOURCE === 'firefox') {
        return function (message) {
            try {
                bdiagnostic.log("sending", message);
                chrome.runtime.sendMessage(null, message, _ => {
                    if (chrome.runtime.lastError) {
                        bdiagnostic.log(chrome.runtime.lastError.message);
                    }
                });
            } catch (ex) {
                bdiagnostic.exception(ex);
            }
        };
    }
	return function (message) {
		bdiagnostic.exception("invalid SendMessageToBackground being used");
	};
})();

var ListenForMessageFromBackground = (function() {
    if (BROWSER_VALUES.SOURCE === 'chrome') {  
        return function (callback) {
            chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
                bdiagnostic.log("received", request);
                callback(request, sendResponse);
            });
        };
    }
    if (BROWSER_VALUES.SOURCE === 'firefox') {  
        return function (callback) {
            chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
                bdiagnostic.log("received", request);
                callback(request, sendResponse);
            });
        };
    }
	return function (callback) {
		bdiagnostic.exception("invalid ListenForMessageFromBackground being used");
	};
})();

var FormAttrs = [
    "action"
    , "class"
    , "data-bind"
    , "data-reactid"
    , "id"
    , "name"
    , WTX_VALUES.CONTEXT
    , WTX_VALUES.FORM_CONTEXT

];

var InputAttrs = [
    "class"
    , "data-bind"
    , "data-bind-class-invalid"
    , "data-bind-hide"
    , "data-bind-value"
    , "data-ng-model"
    , "data-reactid"
    , "id"
    , "multiple" //:bool
    , "name"
    , "ng-model"
    , "ng-model-options"
    , "value"
    , WTX_VALUES.CONTEXT
    , WTX_VALUES.FORM_CONTEXT
    , "placeholder"
];


var SelectAttrs = [
    "data-bind"
    , "id"
    , "multiple" //:bool
    , "name"
    , "ng-model"
    , WTX_VALUES.CONTEXT
    , WTX_VALUES.FORM_CONTEXT
];

var RuleAttrs = [
    WTX_VALUES.RULE_V3
    , WTX_VALUES.RULE_CHECKED_V3
    , WTX_VALUES.RULE_TEXT_V3
    , WTX_VALUES.HAS_PWD_GEN
];

var hidden = "hidden";
var visibilityChange = "visibilitychange";


var OVERRIDES = [
    {
        name: "Central",
        flowSteps: [
            {
                url: "https://login.bitdefender.com/central/login.html?",
                elements: ["{{login.EmailAddress}}"],
                values: {},
            },
            {
                url: "https://login.bitdefender.com/central/password.html?",
                elements: ["{{login.Password}}"],
                values: {},
            },
        ],

    }
];

function checkForOverride(url) {
    let urlToCheck = url;
    for (let ov of OVERRIDES) {
        let index = 0;
        for (let flow of ov.flowSteps) {
            if (urlToCheck.indexOf(flow.url) === 0) {
                return {
                    override: ov,
                    index: index,
                };
            }
            index++;
        }
    }
    return null;
}




//
//  Uuid
//
function UuidCreate() {
    var s = [];
    var hexDigits = "0123456789abcdef";
    for (var i = 0; i < 36; i++) {
        s[i] = hexDigits.substr(Math.floor(Math.random() * 0x10), 1);
    }
    s[14] = "4";
    s[19] = hexDigits.substr((s[19] & 0x3) | 0x8, 1);
    s[8] = s[13] = s[18] = s[23] = "-";

    var uuid = s.join("").toUpperCase();
    return uuid;
}


function getElementProp(element, propertyName, defaultOnNull) {
    var v = "";

    try {
        v = element.getAttribute(propertyName);
        if (!v) {
            v = defaultOnNull;
        }
    } catch (ex) {
        v = "";
    }
    return v;
}



function getElementRect(elem) {
    var pa = { left: 0, top: 0, right: 0, bottom: 0 },
        doc = elem && elem.ownerDocument;

    if (!doc) {
        return pa;
    }

    var docElem = doc.documentElement || document.body.parentNode || doc.body;
    var win = doc.defaultView || doc.parentWindow;

    var bbox = { top: 0, left: 0 };

    if (typeof elem.getBoundingClientRect !== "undefined") {
        bbox = elem.getBoundingClientRect();
    }

    var xscroll = null;
    var yscroll = null;
    if (!win) {
        try {
            var style = window.getComputedStyle(elem);
            if (docElem.scrollLeft === 0) {
                xscroll = parseInt(style.getPropertyValue("width"));
            }
            if (docElem.scrollTop === 0) {
                yscroll = parseInt(style.getPropertyValue("height"));
            }
        }
        catch (ex) {
            xscroll = docElem.scrollLeft;
            yscroll = docElem.scrollTop;
        }
    }
    else {
        xscroll = win.pageXOffset || docElem.scrollLeft;
        yscroll = win.pageYOffset || docElem.scrollTop;
    }

    pa.top = bbox.top + yscroll - docElem.clientTop;
    pa.bottom = pa.top + bbox.height;
    pa.left = bbox.left + xscroll - docElem.clientLeft;
    pa.right = pa.left + bbox.width;

    return pa;
}

function getElementMaxZIndex(elem) {
    var maxZIndex = 0;
    if (elem.children) {
        for (var i = 0; i < elem.children.length; i++) {
            var child = elem.children[i];
            var zIndex = 0;
            try {
                var style = window.getComputedStyle(child);
                zIndex = parseInt(style.getPropertyValue("z-index"));
            }
            catch (ex) {
                zIndex = parseInt(child.style.zIndex);
            }
            if (zIndex && (maxZIndex < zIndex)) {
                maxZIndex = zIndex;
            }
        }
    }
    return maxZIndex;
}

/*
<a style="background-image: url(data:image/png;base64,);
        height:22px;
        width:20px;   
        display:block;
        padding:0px;
        margin:0px;
        border:0px;
        " href="javascript:void(0)">  
</a>

http://www.freeformatter.com/javascript-escape.html
*/


var walletButtonsInPage = [];


function repositionWalletButtons() {
    var failedButtonsIndexes = [];
    for (let i = 0; i < walletButtonsInPage.length; i++) {
        try {
            var buttonInfo = walletButtonsInPage[i];
            var iel = buttonInfo.iel;
            var button = buttonInfo.button;
            var container = buttonInfo.container;
            var containerPosition = getElementRect(container);
            var inputPosition = getElementRect(iel);

            if ((inputPosition.bottom <= inputPosition.top) || (inputPosition.right <= inputPosition.left)) {
                button.style.display = "none";
                return;
            }


            var finalTop = (inputPosition.top - containerPosition.top);
            var finalLeft = (inputPosition.right - containerPosition.left);

            button.style.top = "" + finalTop + "px";
            button.style.left = "" + finalLeft + "px";

            var buttonPosition = getElementRect(button);
            var buttonHeight = buttonPosition.bottom - buttonPosition.top;
            var buttonWidth = buttonPosition.right - buttonPosition.left;
            var inputHeight = inputPosition.bottom - inputPosition.top;
            finalTop = finalTop + 2 + (inputHeight - buttonHeight) / 2;
            finalLeft = finalLeft - buttonWidth - 3;
            button.style.top = "" + finalTop + "px";
            button.style.left = "" + finalLeft + "px";
            button.style.display = "block";
        } catch (ex) {
            bdiagnostic.exception(ex);
            bdiagnostic.log("Button can't we worked on, removing", ex);
            failedButtonsIndexes.push(i);
        }
    }
    for (let i = failedButtonsIndexes.length - 1; i >= 0; i--) {
        walletButtonsInPage.splice(failedButtonsIndexes[i], 1);
    }
}

function toggleWalletButtons(enable) {
    var failedButtonsIndexes = [];
    for (let i = 0; i < walletButtonsInPage.length; i++) {
        try {
            var buttonInfo = walletButtonsInPage[i];
            var container = buttonInfo.container;
            container.style.display = enable ? "block" : "none";
        } catch (ex) {
            bdiagnostic.exception(ex);
            bdiagnostic.log("Button can't we worked on, removing", ex);
            failedButtonsIndexes.push(i);
        }
    }
    for (let i = failedButtonsIndexes.length - 1; i >= 0; i--) {
        walletButtonsInPage.splice(failedButtonsIndexes[i], 1);
    }
    repositionWalletButtons();
}

function updateWalletButtons() {
    SendMessageToBackground({
        verb: "update-buttons-status",
        data: null
    });
}


var completedForms = {};

function onsubmitForm(e) {
    try {
        if (e.target.hasAttribute(WTX_VALUES.CONTEXT)) {
            var formContext = e.target.getAttribute(WTX_VALUES.CONTEXT);
            var reply = getDOMInfo2({ formCtx: formContext, addRules: true });
            reply[WTX_VALUES.VERB] = "before-navigate";
            reply[WTX_VALUES.FORM_CONTEXT] = formContext;
            SendMessageToBackground({
                verb: "info-onsubmit",
                data: reply
            });
        }
    } catch (ex) {
        bdiagnostic.exception(ex);
    }
}
function onclickInput(e) {
    try {
        if (e.target.hasAttribute(WTX_VALUES.CONTEXT)) {
			var formContext;
            if (e.target.form) {
                formContext = e.target.form.getAttribute(WTX_VALUES.CONTEXT);
            }
            else {
                formContext = e.target.ownerDocument.documentElement.getAttribute(WTX_VALUES.CONTEXT);
            }
            if (!completedForms[formContext]) {
                completedForms[formContext] = true;
                var reply = getDOMInfo2({ formCtx: formContext });
                reply[WTX_VALUES.VERB] = "doc-complete";
                reply[WTX_VALUES.FORM_CONTEXT] = formContext;
                reply[WTX_VALUES.CLICK_CONTEXT] = e.target.getAttribute(WTX_VALUES.CONTEXT);
                SendMessageToBackground({
                    verb: "info-onclick",
                    data: reply
                });
            }
        }
    } catch (ex) {
        bdiagnostic.exception(ex);
    }
}



var docList = [];

var defaultPageValues = {};
function saveDefaultValue(iel) {
    try {
        if (iel.value && iel.value !== "" && iel.hasAttribute(WTX_VALUES.CONTEXT) && iel.ownerDocument.documentElement.hasAttribute(WTX_VALUES.CONTEXT)) {
            var docCtx = iel.ownerDocument.documentElement.getAttribute(WTX_VALUES.CONTEXT);
            var inputCtx = iel.getAttribute(WTX_VALUES.CONTEXT);
            if (!defaultPageValues[docCtx]) {
                defaultPageValues[docCtx] = {};
            }
            if (!defaultPageValues[docCtx][inputCtx]) {
                defaultPageValues[docCtx][inputCtx] = iel.value;
            }
        }
    } catch (ex) {
        bdiagnostic.exception(ex);
    }
}
function getDefaultValue(iel) {
    try {
        if (iel.ownerDocument.documentElement.hasAttribute(WTX_VALUES.CONTEXT)) {
            if (iel.hasAttribute(WTX_VALUES.CONTEXT)) {
                var docCtx = iel.ownerDocument.documentElement.getAttribute(WTX_VALUES.CONTEXT);
                var inputCtx = iel.getAttribute(WTX_VALUES.CONTEXT);
                if (defaultPageValues[docCtx] && defaultPageValues[docCtx][inputCtx]) {
                    return defaultPageValues[docCtx][inputCtx];
                }
            }
        }
    } catch (ex) {
        bdiagnostic.exception(ex);
    }
    return "";
}

function clearDefaultValues(del) {
    try {
        if (del.hasAttribute(WTX_VALUES.CONTEXT)) {
            var docCtx = del.getAttribute(WTX_VALUES.CONTEXT);
            if (defaultPageValues[docCtx]) {
                delete defaultPageValues[docCtx];
            }
        }
    } catch (ex) {
        bdiagnostic.exception(ex);
    }
}


function getDocumentsToScan() {
    var documentsToScan = [document];
    //var frames = document.defaultView.frames;
    //var length = frames.length;
    //for (var c = 0; c < length; c++) {
    //    var currentFrame = frames[c];
    //    try {
    //        documentsToScan.push(currentFrame.contentWindow.document);
    //    }
    //    catch (ex) {
    //        bdiagnostic.exception(ex);
    //    }
    //}
    return documentsToScan;
}

function walkAllElements(element, tagName) {
    if (element.getElementsByTagName) {
        var nodeList = element.getElementsByTagName(tagName);
        var length = nodeList.length;
        for (var i = 0; i < length; i++) {
            var el = nodeList[i];
            var arLength = arguments.length;
            for (var a = 2; a < arLength; a++) {
                var callback = arguments[a];
                try {
                    var ret = callback(el);
                    if (ret) {
                        break;
                    }
                } catch (ex) {
                    bdiagnostic.exception(ex);
                }
            }
        }
    }
}

function setupWtxContext(element) {
    if (element.hasAttribute(WTX_VALUES.CONTEXT)) {
        return false;
    } else {
        element.setAttribute(WTX_VALUES.CONTEXT, UuidCreate());
        return true;
    }
}

function addRulesToCompleteElementOverride(elem, currentFlow, reply) {
    delete currentFlow.values[WTX_VALUES.ITEM_ID];
    bdiagnostic.devLog("Preparing to save a value", reply);
    
    for (let form of reply.forms) {
        if (form[WTX_VALUES.CONTEXT] === elem[WTX_VALUES.FORM_CONTEXT]
            && form[WTX_VALUES.ITEM_ID]) {

            currentFlow.values[WTX_VALUES.ITEM_ID] = form[WTX_VALUES.ITEM_ID];
            bdiagnostic.devLog("Saving itemid for later", currentFlow);
            break;
        }
    }

    if (elem.value) {
        currentFlow.values[matchElem] = elem;
        bdiagnostic.devLog("Saving elem for later", currentFlow);
    }

    currentFlow.values["url"] = reply[WTX_VALUES.PARENT_URL];
}

function overrideFormToCompleteFlow(currentFlow, reply, addRules) {
    let formToComplete = null;
    for (let matchElem of currentFlow.elements) {
        for (let elem of reply.elements) {
            if (matchElem === elem[WTX_VALUES.RULE_V3]) {
                if (addRules) {
                    addRulesToCompleteElementOverride(elem, currentFlow, reply);
                }
                formToComplete = elem[WTX_VALUES.FORM_CONTEXT];
            }
        }
    }
    return formToComplete;
}

function completeFormOverride(formToComplete, overrideInfo, reply) {
    if (!formToComplete) {
        return;
    }

    for (let i = 0; i < overrideInfo.index; ++i) {
        let flow = overrideInfo.override.flowSteps[i];
        for (let prop of Object.keys(flow.values)) {
            if (prop === WTX_VALUES.ITEM_ID) {
                //reply["doc-autofill-more-data"] = formToComplete;
                //reply[WTX_VALUES.FORM_CONTEXT] = formToComplete;
                reply[WTX_VALUES.ITEM_ID] = flow.values[WTX_VALUES.ITEM_ID];
            }
            let value = flow.values[prop];
            bdiagnostic.devLog("adding additional info", value);
            let newElem = Object.assign({}, value);
            newElem[WTX_VALUES.FORM_CONTEXT] = formToComplete;
            reply.elements.push(newElem);
        }
    }
}

function overrideDOMReply(reply, addRules) {
    try {
        let overrideInfo = checkForOverride(reply[WTX_VALUES.PARENT_URL]);
        if (!overrideInfo) {
            return;
        }

        bdiagnostic.devLog("Override found", overrideInfo);

        let currentFlow = overrideInfo.override.flowSteps[overrideInfo.index];
        let formToComplete = overrideFormToCompleteFlow(currentFlow, reply, addRules);
        bdiagnostic.devLog("formToComplete", formToComplete);

        for (let i = overrideInfo.index + 1; i < overrideInfo.override.flowSteps.length; ++i) {
            let flow = overrideInfo.override.flowSteps[i];
            flow.values = {};
        }
        
        completeFormOverride(formToComplete, overrideInfo, reply);

        let url = null;
        for (let flow of overrideInfo.override.flowSteps) {
            if (flow.values.url) {
                url = flow.values.url;
                break;
            }
        }

        if (url) {
            reply[WTX_VALUES.PARENT_URL] = url;
        }

        bdiagnostic.devLog("Reply from overrideDOMReply", reply);
    } catch (ex) {
        bdiagnostic.devLog(ex);
    }
}

function getDOMDocuments(parameters) {
    var documents;
    var includeFrames;
    if (parameters) {
        if (parameters.documents) {
            documents = parameters.documents;
        }
        if (parameters.includeFrames) {
            includeFrames = parameters.includeFrames;
        }
    }
    if (!documents) {
        if (includeFrames) {
            documents = getDocumentsToScan();
        } else {
            documents = [document];
        }
    }
    return documents;
}

function setupDOMDocumentReply(reply, mainDocument) {

    var existingContext = mainDocument.documentElement.getAttribute(WTX_VALUES.CONTEXT);
    if (!existingContext) {
        existingContext = "<none>";
    }
    bdiagnostic.log("setupDOMDocumentReply called, existingContext:" + existingContext);
    if (setupWtxContext(mainDocument.documentElement)) {
        mainDocument.documentElement.addEventListener("submit", onsubmitForm, true);
        mainDocument.documentElement.addEventListener("Bdwtsubmit", onsubmitForm, true);
    }
    var topmostTitle = '';
	var topmostTitleError = "false";

    var topmostUrl = '';
	var topmostUrlError = "false";

    try {
        topmostTitle = mainDocument.title;
        topmostUrl = mainDocument.documentURI;
        var topmostWindow = mainDocument.defaultView;
        while (topmostWindow && topmostWindow !== topmostWindow.parent) {
            topmostWindow = topmostWindow.parent;
            try {
                topmostTitle = topmostWindow.document.title;
            }
            catch (ex) {
				topmostTitleError = "true";
            }
            try {
                topmostUrl = topmostWindow.document.documentURI;
            }
            catch (ex) {
				topmostUrlError = "true";
            }
        }
    }
    catch (ex) {
        topmostTitleError = "true";
        topmostUrlError = "true";
        bdiagnostic.exception(ex);
    }

    reply[WTX_VALUES.CONTEXT] = mainDocument.documentElement.getAttribute(WTX_VALUES.CONTEXT);
    reply[WTX_VALUES.TITLE] = topmostTitle;
    reply[WTX_VALUES.BEFORE_URL] = mainDocument.documentURI;
    if (topmostUrl === '') {
        reply[WTX_VALUES.PARENT_URL] = mainDocument.documentURI;
		topmostUrlError = "true";
    } else {
        reply[WTX_VALUES.PARENT_URL] = topmostUrl;
    }

	reply[WTX_VALUES.TITLE_ERROR] = topmostTitleError;
	reply[WTX_VALUES.PARENT_URL_ERR] = topmostUrlError;
}

var currentItemId;

function processDomFormElementReply(fel, reply, formCtx, addRules) {
    var newForms = 0;

    if (setupWtxContext(fel)) { newForms++; }

    let reduceFun = function (acc, attr) {
        var val = getElementProp(fel, attr, "");
        if (val !== "") {
            acc[attr] = val;
        }
        return acc;
    };
    
    var entry = FormAttrs.reduce(reduceFun, {});

    if (addRules) {
        entry = RuleAttrs.reduce(reduceFun, entry);
    }

    if (formCtx) {
        if (currentItemId) {
            entry[WTX_VALUES.ITEM_ID] = currentItemId;
        }
    }
    reply["forms"].push(entry);

    return newForms;
}

function processDOMFormElementsReply(reply, currentDocument, formCtx, addRules) {
    var newForms = 0;

    let contextFun = function (fel) {
        if (formCtx) {
            if (fel.getAttribute(WTX_VALUES.CONTEXT) !== formCtx) {
                return true;
            }
        }
        return false;
    };

    walkAllElements(currentDocument, "form", contextFun, function (fel) {
        newForms += processDomFormElementReply(fel, reply, formCtx, addRules);
    });

    if (newForms > 0) {
        reply[WTX_VALUES.NEW_FORMS] = "true";
    }
}

function processDOMInputElementsReply(reply, currentDocument, getContext, addRules) {
    var newElements = 0;
    walkAllElements(currentDocument, "input", getContext, function (iel) {
        if (setupWtxContext(iel)) {
            newElements++;
            saveDefaultValue(iel);
        }
        let reduceFun = function (acc, attr) {
            var val = getElementProp(iel, attr, "");
            if (val !== "") {
                acc[attr] = val;
            }
            return acc;
        };
        var entry = InputAttrs.reduce(reduceFun, {});
        if (iel.type) {
            entry["type"] = iel.type;
        }

        if (addRules) {
            entry = RuleAttrs.reduce(reduceFun, entry);
        }

        entry[WTX_VALUES.TYPE] = WTX_VALUES.INPUT;

        entry[WTX_VALUES.FORM_CONTEXT] = reply[WTX_VALUES.CONTEXT];
        if (iel.form) {
            entry[WTX_VALUES.FORM_CONTEXT] = iel.form.getAttribute(WTX_VALUES.CONTEXT);
        }

        entry["value"] = iel.value;
        var defValue = getDefaultValue(iel);
        if (defValue) {
            entry[WTX_VALUES.DEF_VALUE] = defValue;
        }

        reply["elements"].push(entry);
    });
    if (newElements > 0) {
        reply[WTX_VALUES.NEW_ELEMENTS] = "true";
    }
}

function processDOMSelectElementsReply(reply, currentDocument, getContext, addRules) {
    var newElements = 0;
    walkAllElements(currentDocument, "select", getContext, function (sel) {
        if (setupWtxContext(sel)) { newElements++; }
        let reduceFun = function (acc, attr) {
            var val = getElementProp(sel, attr, "");
            if (val !== "") {
                acc[attr] = val;
            }
            return acc;
        };
        var entry = SelectAttrs.reduce(reduceFun, {});
        if (sel.type) {
            entry["type"] = sel.type;
        }

        if (addRules) {
            entry = RuleAttrs.reduce(reduceFun, entry);
        }

        entry[WTX_VALUES.FORM_CONTEXT] = reply[WTX_VALUES.CONTEXT];
        if (sel.form) {
            entry[WTX_VALUES.FORM_CONTEXT] = sel.form.getAttribute(WTX_VALUES.CONTEXT);
        }

        entry[WTX_VALUES.TYPE] = WTX_VALUES.SELECT;
        reply["elements"].push(entry);
    });
    if (newElements > 0) {
        reply[WTX_VALUES.NEW_ELEMENTS] = "true";
    }
}

function getDOMInfo2(parameters) {

    var reply = {};
    reply[WTX_VALUES.SOURCE] = BROWSER_VALUES.SOURCE;
    reply["elements"] = [];
    reply["forms"] = [];

    var documents = getDOMDocuments(parameters);
    var mainDocument = documents[0];
    setupDOMDocumentReply(reply, mainDocument);

    var formCtx;
    var addRules = false;
    if (parameters) {
        if (parameters.formCtx) {
            formCtx = parameters.formCtx;
        }
        if (parameters.addRules) {
            addRules = parameters.addRules;
        }
    }

    let getContext = function (iel) {
        if (formCtx) {
            var ctx = null;
            if (iel.form) {
                ctx = iel.form.getAttribute(WTX_VALUES.CONTEXT)
            } else {
                ctx = iel.ownerDocument.documentElement.getAttribute(WTX_VALUES.CONTEXT);
            }
            if (formCtx !== ctx) {
                return true;
            }
        }
        return false;
    };

    documents.reduce(function (acc, currentDocument) {
        
        processDOMFormElementsReply(reply, currentDocument, formCtx, addRules);
        processDOMInputElementsReply(reply, currentDocument, getContext, addRules);
        processDOMSelectElementsReply(reply, currentDocument, getContext, addRules);

    }, null);

    overrideDOMReply(reply, addRules);

    return reply;
}

function getWalletButtonTemplate(currentDocument) {
    var walletButtonTemplate = currentDocument.createElement("div");
    walletButtonTemplate.innerHTML = "<a style=\"background-image: url(data:image\/png;base64,);\r\n            height:22px;\r\n            width:20px;   \r\n          \tdisplay:block;\r\n\t\t\tpadding:0px;\r\n\t\t\tmargin:0px;\r\n\t\t\tborder:0px;\r\n            \" href=\"javascript:void(0)\">  \r\n\t<\/a>";
    walletButtonTemplate = walletButtonTemplate.firstChild;
    return walletButtonTemplate;
}

function installWalletButton(currentDocument, iel) {
    iel.setAttribute(WTX_VALUES.HAS_PWD_GEN, "true");
    bdiagnostic.log("Installing wtx buttons");

    var container = currentDocument.createElement("div");
    container.style.position = "relative";
    container.style.width = 0;
    container.style.width = 0;
    container.style.margin = 0;
    container.style.padding = 0;
    container.style.display = "none";

    var template = getWalletButtonTemplate(currentDocument);
    var button = template.cloneNode(true);
    container.appendChild(button);
    button.style.position = "absolute";
    button.style.zIndex = "" + (getElementMaxZIndex(iel.parentNode) + 999);

    (function (iel, container, button) {
        button.addEventListener("mousedown", function () {
            bdiagnostic.log("iel.focus()");
            setTimeout(function () { iel.focus(); }, 0);
            var doc = iel.ownerDocument;
            var fcontext = iel.form ? iel.form.getAttribute(WTX_VALUES.CONTEXT) : doc.documentElement.getAttribute(WTX_VALUES.CONTEXT);
            var reply = getDOMInfo2({ documents: [doc], formCtx: fcontext });
            reply[WTX_VALUES.VERB] = "doc-password-generator";
            reply[WTX_VALUES.FORM_CONTEXT] = fcontext;
            SendMessageToBackground({
                verb: "request-generate-password",
                data: reply
            });
        });

        iel.parentNode.insertBefore(container, iel.nextSibling);
        walletButtonsInPage.push({ iel: iel, container: container, button: button });

    })(iel, container, button);
}

function checkInputOnDocComplete(iel, info, buttonCheck, currentDocument) {
    if (iel.type !== "submit") {
        if (info[WTX_VALUES.HAS_PWD_GEN] === "true") {
            var fcontext = iel.form ? 
                iel.form.getAttribute(WTX_VALUES.CONTEXT) :
                doc.documentElement.getAttribute(WTX_VALUES.CONTEXT);

            if (buttonCheck[fcontext]) {
                return;
            }
            buttonCheck[fcontext] = true;

            var prevAttr = getElementProp(iel, WTX_VALUES.HAS_PWD_GEN, "");
            if (prevAttr !== "true") {
                installWalletButton(currentDocument, iel);
            }
        }
    }
}

function checkLoginRuleOnDocComplete(iel, info) {
    let ruleType = info[WTX_VALUES.RULE_V3];
    if (ruleType && (ruleType.lastIndexOf("{{login.", 0) !== 0) && (iel.type !== "submit")) {
        iel.addEventListener("click", onclickInput, false);
        //(function () {
        //    var timedEvent = null;
        //    var dContext = iel.ownerDocument.documentElement.getAttribute(WTX_VALUES.CONTEXT);
        //    var fContext = dContext;
        //    if (iel.form) {
        //        fContext = iel.form.getAttribute(WTX_VALUES.CONTEXT);
        //    }
        //    var iContext = iel.getAttribute(WTX_VALUES.CONTEXT);
        //    iel.addEventListener("input", function (e) {
        //        if (timedEvent !== null) {
        //            clearTimeout(timedEvent);
        //            timedEvent = null;
        //        }
        //        CredStore.SetCredentials(dContext, iContext, e.target.value);
        //    });
        //}());
    }
    //else if (ruleType.lastIndexOf("{{login.", 0) === 0) {
    //}
}

function completeWithAttributes(el, info) {
    if (info && info[WTX_VALUES.RULE_V3]) {
        var oldProp = getElementProp(el, WTX_VALUES.RULE_V3, "");
        if (oldProp !== info[WTX_VALUES.RULE_V3]) {
            el.setAttribute(WTX_VALUES.RULE_V3, info[WTX_VALUES.RULE_V3]);
            return false;
        }
    }
    return true;
}

function processDocComplete(currentDocument, runIfMatch) {

    var newForms = {};
    var setElementFormOfInterest = function (el) {
        if (el.form) {
            newForms[el.form.getAttribute(WTX_VALUES.CONTEXT)] = true;
        }
    };

    var buttonCheck = {};
    walkAllElements(currentDocument, "input",
        runIfMatch(function (iel, info) {
            checkInputOnDocComplete(iel, info, buttonCheck, currentDocument);
        }),
        runIfMatch(function (iel, info) {
            completeWithAttributes(iel, info);
        }),
        runIfMatch(function (iel, info) {
            checkLoginRuleOnDocComplete(iel, info);
        }),
        setElementFormOfInterest);

    walkAllElements(currentDocument, "select",
        function (iel, info) {
            completeWithAttributes(iel, info);
        },
        setElementFormOfInterest);

    for (var newFormCtx in newForms) {
        if (newForms.hasOwnProperty(newFormCtx)) {
            var doc = (currentDocument.head || currentDocument.documentElement);
            var form = document.querySelector('form[wtx-context="' + newFormCtx + '"]');
            if (form) {
                form.submit = (function (oldSubmit) {
                    return function () {
                        this.dispatchEvent(new CustomEvent('Bdwtsubmit'));
                        return oldSubmit.apply(this, arguments);
                    };
                })(form.submit);
            }
        }
    }
}

function onWtxMsgDocComplete(request, documentsToScan, runIfMatch) {

    documentsToScan.reduce(function (acc, currentDocument) {
        processDocComplete(currentDocument, runIfMatch);
    }, null);

    let params = { addRules: true };
    if (request[WTX_VALUES.FORM_CONTEXT]) {
        params.formCtx = request[WTX_VALUES.FORM_CONTEXT];
    }

    let reply = getDOMInfo2(params);
    reply[WTX_VALUES.VERB] = "doc-autofill";
    if (request["doc-autofill-more-data"]) {
        bdiagnostic.log('Requesting more data instead of autofill');
        reply[WTX_VALUES.VERB] = "doc-autofill-more-data";
        reply["doc-autofill-more-data"] = request["doc-autofill-more-data"];
        currentItemId = null;
    }
    if (request[WTX_VALUES.CLICK_CONTEXT]) {
        reply[WTX_VALUES.CLICK_CONTEXT] = request[WTX_VALUES.CLICK_CONTEXT];
    }
    reply[WTX_VALUES.FORM_CONTEXT] = request[WTX_VALUES.FORM_CONTEXT];
    SendMessageToBackground({
        verb: "info-oncomplete",
        data: reply
    });

    repositionWalletButtons();
    updateWalletButtons();
}

function generateInputEvent(iel, currentDocument) {
    try {
        var inputEvent = currentDocument.createEvent("HTMLEvents");
        inputEvent.initEvent("input", true, true);
        iel.dispatchEvent(inputEvent);
    }
    catch (err) {
        bdiagnostic.log(err);
    }
}

function processInputOnDocAutofill(iel, info, currentDocument) {
    if (info[WTX_VALUES.VALUE]) {
        if (iel.type === "radio") {
            if (info[WTX_VALUES.VALUE] === "false") {
                iel.checked = false;
            }
            iel.checked = true;

            return false;
        }
        else {
            iel.focus();
            iel.value = info[WTX_VALUES.VALUE];
            generateInputEvent(iel, currentDocument);
            iel.blur();

            return true;
        }
    }
    return false;
}

function processSelectOnDocAutofill(el, info) {
    let optionFun = function (opt) {
        var value = opt.value;
        var text = opt.textContent;
        var lvalue = value.toLowerCase();
        var ltext = text.toLowerCase();
        var linfo = info[WTX_VALUES.VALUE].toLowerCase();

        if (linfo === "iasi") {
            return;
        }

        if (ltext === linfo || lvalue === linfo) {
            el.value = value;
        }
    };
    if (info[WTX_VALUES.VALUE]) {
        walkAllElements(el, "option", optionFun);
        //el.setAttribute("value", info[WTX_VALUES.VALUE]);
    }
}

function onWtxMsgDocAutofill(request, documentsToScan, runIfMatch) {
    currentItemId = false;

    let formFun = function (iel, info) {
        if (info[WTX_VALUES.ITEM_ID] && !request["doc-autofill-more-data"]) {
            bdiagnostic.log("wtx-item-id found");
            currentItemId = info[WTX_VALUES.ITEM_ID];
            return true;
        }
        return false;
    };

    documentsToScan.reduce(function (acc, currentDocument) {
        walkAllElements(currentDocument, "form",
            runIfMatch(formFun));

        var firstInput = null;
        walkAllElements(currentDocument, "input",
            runIfMatch(function (iel, info) {
                let valueInput = processInputOnDocAutofill(iel, info, currentDocument);
                if (!firstInput && valueInput) {
                    firstInput = iel;
                }            
            }));

        if (firstInput) {
            firstInput.focus();
            generateInputEvent(firstInput, currentDocument);
            firstInput.blur();
        }

        walkAllElements(currentDocument, "select",
            runIfMatch(processSelectOnDocAutofill));

    }, null);
}

function onWtxMsgDocPwnGen(request, documentsToScan) {
    documentsToScan.reduce(function (acc, currentDocument) {
        walkAllElements(currentDocument, "form", function (fel) {
            var fcontext = getElementProp(fel, WTX_VALUES.CONTEXT, "");
            if (fcontext === request[WTX_VALUES.FORM_CONTEXT]) {
                walkAllElements(fel, "input", function (iel) {
                    if (iel.type === "password") {
                        iel.focus();
                        iel.value = request["wnd::password"];
                        generateInputEvent(iel, currentDocument);
                        iel.blur();
                    }
                });
            }
        });
    }, null);
}

function onWtxMsgBeforeNavigate(request, documentsToScan)  {
    if (request[WTX_VALUES.FORM_CONTEXT]) {
        var moreDataNeeded = false;
        documentsToScan.reduce(function (acc, currentDocument) {
            walkAllElements(currentDocument, "form", function (fel) {
                var fcontext = getElementProp(fel, WTX_VALUES.CONTEXT, "");
                if (fcontext === request[WTX_VALUES.FORM_CONTEXT]) {
                    moreDataNeeded = true;
                    return true;
                }
            });
        }, null);
        if (moreDataNeeded) {
            bdiagnostic.devLog("More data needed");
            let params = {};
            params.formCtx = request[WTX_VALUES.FORM_CONTEXT];
            let reply = getDOMInfo2(params);
            reply[WTX_VALUES.VERB] = "doc-complete";
            reply[WTX_VALUES.FORM_CONTEXT] = request[WTX_VALUES.FORM_CONTEXT];
            reply["doc-autofill-more-data"] = request[WTX_VALUES.FORM_CONTEXT];
            SendMessageToBackground({
                verb: "info-oncomplete",
                data: reply
            });
        }
    }
}

function onWtxMsgHandleMenuRsp(request) {
    let params = { addRules: true };
    currentItemId = request[WTX_VALUES.ITEM_ID];
    let reply = getDOMInfo2(params);
    reply[WTX_VALUES.ITEM_ID] = request[WTX_VALUES.ITEM_ID];
    reply[WTX_VALUES.VERB] = "doc-autofill";
    SendMessageToBackground({
        verb: "info-oncomplete",
        data: reply
    });
}

function onWtxVerbMessage(request, sendResponse) {
    var elementsMap = {};
    var maxI = request.elements.length;
    for (let i = 0; i < maxI; i++) {
        let el = request.elements[i];
        elementsMap[el[WTX_VALUES.CONTEXT]] = el;
    }

    maxI = request.forms.length;
    for (let i = 0; i < maxI; i++) {
        let el = request.forms[i];
        elementsMap[el[WTX_VALUES.CONTEXT]] = el;
    }

    function runIfMatch(callback) {
        return function (el) {
            var context = getElementProp(el, WTX_VALUES.CONTEXT, null);
            if (context) {
                var info = elementsMap[context];
                if (info) {
                    return callback(el, info);
                }
            }
            return true;
        }
    }

    var documentsToScan = getDocumentsToScan();

    var verb = request[WTX_VALUES.VERB];
    if (verb === "doc-complete") {
        onWtxMsgDocComplete(request, documentsToScan, runIfMatch);
    }
    else if (verb === "doc-autofill") {
        onWtxMsgDocAutofill(request, documentsToScan, runIfMatch);
    }
    else if (verb === "doc-password-generator-rsp") {
        onWtxMsgDocPwnGen(request, documentsToScan);
    }
    else if (verb === "before-navigate") {
        onWtxMsgBeforeNavigate(request, documentsToScan);
    }
    else if (verb === "handle-menu-rsp") {
        onWtxMsgHandleMenuRsp(request);
    }
    else {
        let reply = {
            "type": "verb-unknown",
            "data": request
        };
        sendResponse(reply);
    }
}

ListenForMessageFromBackground(function (request, sendResponse) {
    bdiagnostic.log(request);

    var verb = request[WTX_VALUES.VERB];
    if (verb) {
        onWtxVerbMessage(request, sendResponse);
    }
    else if (request.verb) {
        verb = request.verb;
        if (verb === "hide-wallet-controls") {
            bdiagnostic.log("hide-wallet-controls caled");
            toggleWalletButtons(false);
        }
        else if (verb === "show-wallet-controls") {
            bdiagnostic.log("show-wallet-controls caled");
            toggleWalletButtons(true);
        }
        else if (verb === "before-navigate-event") {
            let reply = getDOMInfo2({ addRules: true });
            reply[WTX_VALUES.VERB] = "before-navigate";
            SendMessageToBackground({
                verb: "info-onsubmit",
                data: reply
            });
        }
    }
});


function onVisible(e) {
    try {
        if (!document[hidden]) {
            var reply = getDOMInfo2({ documents: [document] });
            reply[WTX_VALUES.VERB] = "doc-complete";
            SendMessageToBackground({
                verb: "info-oncomplete",
                data: reply
            });
            document.removeEventListener(visibilityChange, onVisible);
        }
    } catch (ex) {
        bdiagnostic.exception(ex);
    }
}

function processDocument(document) {
    try {
        if (!document[hidden]) {
            var reply = getDOMInfo2({ documents: [document] });
            reply[WTX_VALUES.VERB] = "doc-complete";
            SendMessageToBackground({
                verb: "info-oncomplete",
                data: reply
            });
        } else {
            document.addEventListener(visibilityChange, onVisible, false);
        }

    } catch (ex) {
        bdiagnostic.exception(ex);
    }
}

function onComplete(e) {

    processDocument(document);

    document.addEventListener(visibilityChange, function () {
        if (!document[hidden]) {
            updateWalletButtons();
        }
    }, false);

    // select the target node
    var target = document.documentElement;

    // create an observer instance
    var observer = new document.defaultView.MutationObserver(function (mutations) {
        var somethingNew = false;
        function test(node) {
            if (node.tagName === "FORM" || node.tagName === "INPUT" || node.tagName === "SELECT") {
                somethingNew = true;
                if (node.hasAttribute(WTX_VALUES.CONTEXT)) {
                    node.removeAttribute(WTX_VALUES.CONTEXT);
                }
                bdiagnostic.log("somethingNew set to true");
                return true;
            }
        }

        mutations.forEach(function (mutation) {
            for (var i = 0; i < mutation.addedNodes.length; i++) {
                var node = mutation.addedNodes[i];
                test(node);
                walkAllElements(node, "form", test);
                walkAllElements(node, "input", test);
                walkAllElements(node, "select", test);
            }
        });
        if (somethingNew) {
            bdiagnostic.log("Processing...");
            processDocument(document);
        }
    });

    // configuration of the observer:
    var config = { childList: true, subtree: true };

    // pass in the target node, as well as the observer options
    observer.observe(target, config);
}

function pageScript(event) {
    window.addEventListener("resize", repositionWalletButtons);
    window.addEventListener("unload", function (event) {
    });

    if (typeof document.webkitHidden !== "undefined") {
        hidden = "webkitHidden";
        visibilityChange = "webkitvisibilitychange";
    }


    var totalForms = 0;

    if (document.readyState === "complete" || document.readyState === "interactive") {
        onComplete(event);
    } else {
        bdiagnostic.log("shouldn't arrive here");
        window.addEventListener("DOMContentLoaded", onComplete, false);
    }

    return true;
};
bdiagnostic.log("content.js loaded", document);
pageScript();
