var NMH = NMH || function () {
        ...
        try {
            if (msg["nmh_message"]) {
                if (msg["nmh_message"] === "update") {
                    uninitPort();
                    compatAction.setTitle({ title: TooltipStrings.update });
                }
            }
		...
		if (p.error && p.error.message && browser) {
			if (p.error.message.startsWith("No such native application")) {
				browser.management.getSelf().then(function (info) {
					chrome.alarms.create("uninstall-native-self", { when: Date.now() + 500 });
				});
			}
		}
	}
	...
	function uninitPort() {
		...
		compatAction.setTitle({ title: TooltipStrings.restart });
